...
struct ch32v_bc12_data {
	...
	struct k_timer detect_timer;
	struct k_timer vbus_timer;
...
static void ch32v_bc12_notify_callback(const struct device *dev,
...
static bool ch32v_bc12_partner_has_changed(const struct device *dev,
					   struct bc12_partner_state *const state)
{
	struct ch32v_bc12_data *data = dev->data;
	...
	if (state->bc12_role != data->partner_state.bc12_role) {
		return true;
	}

	if (state->bc12_role == BC12_PORTABLE_DEVICE &&
	    data->partner_state.type != state->type) {
		return true;
	}
 ...
 */
static void ch32v_bc12_update_charging_partner(const struct device *dev,
					       struct bc12_partner_state *const state)
{
	struct ch32v_bc12_data *data = dev->data;

	if (!ch32v_bc12_partner_has_changed(dev, state)) {
	...
	if (state) {
		...
		ch32v_bc12_notify_callback(dev, state);
	} else {
		data->partner_state.bc12_role = BC12_DISCONNECTED;
		ch32v_bc12_notify_callback(dev, NULL);
	}
}
...
static int ch32v_bc12_client_detect_start(const struct device *dev)
{

	/* Put pi3usb9201 into client mode */
	data->current_role = BC12_PORTABLE_DEVICE;
	data->detection_state = BC12_STATE_VBUS_DETECT;
	k_timer_start(&data->vbus_timer, K_MSEC(5), K_NO_WAIT);
...
static int ch32v_bc12_disconnect(const struct device *dev)
{
	/* Remove BC1.2 peripherals from data lines */
	ch32v_set_dpdm(dev, BC12_DP_OPEN, BC12_DM_OPEN);

	/* Stop bc1.2 detection */
	k_timer_stop(&data->detect_timer);

	/* Stop VBUS sampling */
	k_timer_stop(&data->vbus_timer);

	/*
	 * Let the application know there's no more charge available for the
	 * supplier type that was most recently detected.
	 */
	ch32v_bc12_update_charging_partner(dev, NULL);

	return 0;
}

static int ch32v_bc12_set_portable_device(const struct device *dev)
{


	if (ch32v_bc12_client_detect_start(dev) < 0) {
		struct bc12_partner_state partner_state;
...
static void ch32v_bc12_work_handler(struct k_work *work)
{
	...
	case BC12_STATE_DATA_CONTACT_DETECT:
		if (dp_level == BC12_LOGIC_HIGH && k_timer_status_get(&data->detect_timer) > 0) {
			data->partner_state.type = BC12_TYPE_UNKNOWN;
			data->detection_state = BC12_STATE_FINISH;
			break;
		}

		/* Data pins make contact */
		if (dp_level != BC12_LOGIC_HIGH) {
			/* Data contact detect debounce finish */
			if (k_timer_status_get(&data->detect_timer) > 0) {
				ch32v_set_dpdm(dev, BC12_DP_VDP_SRC, BC12_DM_IDM_SINK);
				k_timer_start(&data->detect_timer, K_MSEC(BC12_T_VDPSRC_ON_MIN_MS),
					      K_NO_WAIT);
				data->detection_state = BC12_STATE_PRIMARY_DETECTION;
			} else {
				/* Data contact detect debounce start */
				k_timer_start(&data->detect_timer, K_MSEC(BC12_T_DCD_DBNC_MIN_MS),
					      K_NO_WAIT);
			}
		}
	...
	case BC12_STATE_PRIMARY_DETECTION:
		...
		if (k_timer_status_get(&data->detect_timer) > 0) {
			...
			k_timer_start(&data->detect_timer, K_MSEC(BC12_T_VDMSRC_ON_MIN_MS), K_NO_WAIT);
...
static void ch32v_bc12_gpio_callback(const struct device *dev, struct gpio_callback *cb,
				     uint32_t pins)
{
...
static void ch32v_bc12_detect_timer_callback(struct k_timer *timer)
{
	struct ch32v_bc12_data *data = CONTAINER_OF(timer, struct ch32v_bc12_data, detect_timer);
	k_work_submit(&data->work);
}

static void ch32v_bc12_vbus_timer_callback(struct k_timer *timer)
{
	struct ch32v_bc12_data *data = CONTAINER_OF(timer, struct ch32v_bc12_data, vbus_timer);
...
static void ch32v_bc12_adc_callback(const struct device *dev, struct adc_sequence *seq, int err)
{
	...
	const struct ch32v_bc12_config *config = data->dev->config;
	int value = data->adc_raw;

	if (err) {
		return;
	}

	if (data->detection_state == BC12_STATE_IDLE) {
		k_timer_stop(&data->vbus_timer);
		return;
	}

	if (adc_raw_to_millivolts_dt(&config->adc_channel, &value) != 0) {
	...
	if (value >= 2000u && data->detection_state == BC12_STATE_VBUS_DETECT) {
		ch32v_set_dpdm(dev, BC12_DP_IDP_SRC, BC12_DM_RDM_DWN);
		k_timer_start(&data->detect_timer, K_MSEC(BC12_T_DCD_TIMEOUT_MIN_MS), K_NO_WAIT);
...
static int ch32v_bc12_set_role(const struct device *dev, const enum bc12_role role)
{
	...
	case BC12_DISCONNECTED:
		return ch32v_bc12_disconnect(dev);
	case BC12_PORTABLE_DEVICE:
		return ch32v_bc12_set_portable_device(dev);
	default:
...
#define CFG0_PD_C		(VENDOR_CFG0_BASE + 0x02)
	...
	k_timer_init(&data->dcd_timer, ch32v_bc12_dcd_timer_callback, NULL);
	k_timer_init(&data->detect_timer, ch32v_bc12_detect_timer_callback, NULL);
	k_timer_init(&data->vbus_timer, ch32v_bc12_vbus_timer_callback, NULL);
...
#define BC12_WCH_INIT(inst)                                                                        \
	...
	static const struct ch32v_bc12_config drv_config_##inst = {                                \
		...
		.adc_channel = ADC_DT_SPEC_INST_GET(inst),				           \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, ch32v_bc12_init, NULL, &drv_data_##inst,                       \
			      &drv_config_##inst, POST_KERNEL,                                     \
			      USB_BC12_INIT_PRIORITY, &ch32v_bc12_driver_api);

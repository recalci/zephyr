...
The BC1.2 device driver provides four APIs. For devices with autonomous
detection, application software can only use ``bc12_set_role()`` and
``bc12_set_result_cb()``.
...
For devices without autonomous detection, the driver provides ``bc12_set_dpdm()``
and ``bc12_get_dpdm()`` APIs. Zephyr implements logic for controlling and
sensing the DP/DM pins.
